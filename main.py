    ...
    with st.sidebar:
        ...
        with st.expander("Add context for better accuracy"):
            ...
            format_type = st.selectbox(
                "Format Type",
                options=["Auto", "Meeting", "Interview", "Lecture", "Podcast", "Legal", "Medical"],
                key="context_format"
            )
        ...
        # Footer - Minimal
        st.markdown("---")
        st.caption("v2.0 | [Docs](https://github.com) | [API Key](https://makersuite.google.com)")
...
def render_transcript_display():
    ...
    # Display tabs
    tab1, tab2, tab3, tab4 = st.tabs(["📝 Transcript", "✏️ Edit", "📊 Quality", "💾 Export"])
        ...
        if quality.issues:
            ...
            with st.expander("View Issues"):
                for issue in quality.issues[:10]:
                    st.markdown(f"- **{issue.get('type', 'Unknown')}**: {issue.get('message', '')}")
